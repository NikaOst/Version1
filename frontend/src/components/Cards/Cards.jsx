  ...
  return (
    <div className="bg-slate-100">
      <div className="container py-24 px-6 lg:px-12 xl:px-44">
        {/* Section Title */}
        <h2 className="font-sansroboto text-center text-secondary text-xl md:text-2xl font-medium mb-4">
        ...
        <motion.h1
          ...
          whileInView="animate"
          viewport={{ once: true }}
          className="font-sansroboto text-3xl xl:text-4xl font-bold text-center text-primary/90 mb-12">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
          {cardsData.map((card, index) => (
            <motion.div
              ...
              {/* Title */}
              <p className="text-lg text-white font-semibold my-4">{card.title}</p>
      ...
      {/* Modal */}
      {selectedCard && (
        <CardModal isOpen={!!selectedCard} onClose={handleCloseModal} cardData={selectedCard} />
      )}
    </div>
  );
};
